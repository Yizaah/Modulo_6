...
- Registro: `/register/` (vista personalizada en `eventos.views.register`) — formulario mejorado (`eventos/forms.py`) y verificación por correo (email de activación; cuenta inactiva hasta la activación).
- Restablecimiento de contraseña: rutas estándar de Django (`/password_reset/`, etc.) con plantillas en `eventos/templates/registration/`.
...
Nota: Si quieres probar el restablecimiento de contraseña en desarrollo, `settings.py` configura `EMAIL_BACKEND` a `console.EmailBackend` cuando `DEBUG=True`, por lo que los emails aparecerán en la consola del servidor.
